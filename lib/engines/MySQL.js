...
_.fetch = function(struct, cond, option, cb, conn) {
	...
	this._query(query, function(err, results) {
		...
		async.series(
			...
			function(err, results) {
				if (multiInsertData && multiInsertData.multiInsert && multiInsertData.queue.length > 0) {
					that._query(that._buildMultiSaveQuery(multiInsertData), function(err, results) {
						cb(err, results);
					}, conn);
				} else {
					cb(err, results);
				...
				return function(callback) {
					...
					if (query) {						
						if (multiInsertData && multiInsertData.multiInsert) {
							...
							that._query(query, function(err, results) {
...
_.remove = function(struct, data, cb, conn) {
	...
	async.series(
		queries.map(function(q) {
			return function(callback) {
				that._query(q, function(err, results) {
					callback(err, results);
				}, conn);
			};
		}),
...
_._query = function(query, cb, conn) {
	print('Query:', query);
	if (conn) {
...
_._preparedQuery = function (query, params, cb, conn) {
...
_.query = function (query, cb, conn) {
	this._query(query, cb, conn);
};

_.preparedQuery = function (preparedQuery, params, cb, conn) {
	this._preparedQuery(preparedQuery, params, cb, conn);
};
...
_.buildFetchQuery = function(struct, cond, option) {
	...
	if (option.hasOwnProperty('getRemovedData')) {
		if (!cond['removed_at'])
